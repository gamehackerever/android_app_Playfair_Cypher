...
class DecryptionActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        nextButton.setOnClickListener(){
            ...
            } else if ((wordEdit.text.length % 2 != 0)) {
                Toast.makeText(
                    this,
                    "Please Enter Alphabets in Pairs!",
                    Toast.LENGTH_LONG
                ).show()
            } else if (!noAdjacentSame(wordEdit.text.toString())) {
                Toast.makeText(
                    this,
                    "Adjacent Letters cannot be same",
                    Toast.LENGTH_LONG
                ).show()
    ...
    fun noAdjacentSame(input: String): Boolean {
