...
class MatrixDecryption : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_matrix_decryption)
        ...
        mainButton.setOnClickListener() {
            if (count == 0) {
                ...
                if (newCount == ((word.length))) {
                    cypherText.text = removeX(cypherText.text.toString())
    ...
    fun findCipherPairIndices(plaintext: String, matrix: Array<CharArray>, i: Int): Array<IntArray> {
        ...
        if (row1 == row2) {
            col1 = (col1 + 4) % 5 // Move to the right by 1 (taking care of boundary)
            col2 = (col2 + 4) % 5 // Move to the right by 1 (taking care of boundary)
        }
        // Case 2: If both characters are in the same column
        else if (col1 == col2) {
            row1 = (row1 + 4) % 5 // Move down by 1 (taking care of boundary)
            row2 = (row2 + 4) % 5 // Move down by 1 (taking care of boundary)
        }
    ...
    fun removeX(input: String): String {
        val charArray = input.toCharArray()

        for (i in charArray.indices) {
            if (charArray[i] == 'X') {
                if (i > 0 && i < charArray.size - 1 && charArray[i - 1] == charArray[i + 1]) {
                    charArray[i] = ' ' // remove X if the characters on its left and right are the same
                } else if (i == charArray.size - 1) {
                    charArray[i] = ' ' // remove X if it is the last character
                }
            }
        }
        Log.e("TAG", String(charArray).replace(" ", ""))
        return String(charArray).replace(" ", "") // convert char array to string and removee mpty spaces
    }
}